    ...
    public void handle(byte[] data) {
        ...
        while (true) {
            try {
                ...
                ConnectorConfig.socketChannel.read(allocate);
                ...
                File nativeFile = new File(ConnectorConfig.nativeHexoPath+File.separator+ Constant.blogPath +File.separator+fileName);
                ...
                if(nativeFile.length() == fileLength&&nativeFile.exists()){
                    ...
                    ConnectorConfig.socketChannel.write(ByteBuffer.wrap(bytes1));
                    continue;
                }
                System.out.println(ConnectorConfig.nativeHexoPath+File.separator+fileName);
                //写出文件不相同命令
                bytes1[0] = 2;
                ConnectorConfig.socketChannel.write(ByteBuffer.wrap(bytes1));
                ...
                try (OutputStream outputStream = Files.newOutputStream(path)) {
                    while (readLength < fileLength) {
                        int read = ConnectorConfig.socketChannel.read(fileBuffer);
                        readLength += read;
                        fileBuffer.flip();
                ...
                //输出
                //输出到workplace
                File file = new File(ConnectorConfig.nativeHexoPath+File.separator+Constant.cachePath + File.separator + fileName);
